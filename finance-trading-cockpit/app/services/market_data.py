...
async def get_quote(symbol: str) -> Quote:
    ...
    return await _with_provider_fallback("quote", symbol)
...
async def get_history(symbol: str, days: int = 30) -> list[HistoryPoint]:
    ...
    return await _with_provider_fallback("history", symbol, days)
...
async def _search_yahoo_symbols(query: str, limit: int = 8) -> list[SymbolSearchResult]:
...
async def _with_provider_fallback(operation: str, symbol: str, *args):
    ...
    if fallback != "none" and fallback not in providers:
    ...
    for provider in providers:
        try:
            if operation == "quote":
                return await _provider_quote(provider, symbol)
            if operation == "history":
                return await _provider_history(provider, symbol, *args)
            if operation == "history_range":
                return await _provider_history_range(provider, symbol, *args)
        except Exception as exc:
            errors.append(f"{provider}: {exc}")

    raise ValueError("; ".join(errors) or f"No market data provider available for {symbol}")
